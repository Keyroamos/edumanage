...
import {
    ...
    Shield, ChevronRight, ChevronLeft, Save, X, AlertCircle
} from 'lucide-react';
...
const StaffCreate = () => {
    ...
    useEffect(() => {
        const fetchSupervisors = async () => {
            try {
                ...
                // For now, any staff can be a supervisor or we could filter
                setSupervisors(response.data.staff || []);
            } catch (err) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const response = await axios.post('/api/hr/staff/create/', formData);
        ...
        <div className="p-6 md:p-8 bg-slate-50 dark:bg-slate-950 min-h-screen">
            <div className="max-w-4xl mx-auto">
                <div className="flex items-center justify-between mb-8">
                    <div>
                        <h1 className="text-3xl font-bold text-slate-900 dark:text-white">Add New Staff</h1>
                        <p className="text-slate-500 dark:text-slate-400">Register a new non-teaching staff member</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="p-8">
                        {step === 1 && (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <Input label="National ID / Passport" name="national_id" value={formData.national_id} onChange={handleChange} required />
                                    <Input type="date" label="Date of Birth" name="date_of_birth" value={formData.date_of_birth} onChange={handleChange} icon={Calendar} />
                                </div>
                        ...
                        {step === 2 && (
                            ...
                            >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <Input type="date" label="Date Joined" name="date_joined" value={formData.date_joined} onChange={handleChange} icon={Calendar} />
                        ...
                        {step === 3 && (
                            ...
                            >
                                ...
                                <div className="pt-6 border-t border-slate-200 dark:border-slate-800">
                                    <h3 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Other Information</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="flex justify-between mt-12 pt-8 border-t border-slate-200 dark:border-slate-800">
                            ...
                            {step < 3 ? (
                                ...
                                <Button type="submit" isLoading={loading}>
                                    <Save size={20} className="mr-2" /> Save Staff Member
                                </Button>
                            )}
                        </div>
                    </form>
...
const Select = ({ label, name, value, onChange, options, required }) => (
    <div className="space-y-2">
        ...
        <select
            ...
            onChange={onChange}
            required={required}
            className="w-full h-11 rounded-xl border border-slate-200 bg-slate-50 dark:bg-slate-900 dark:border-slate-700 dark:text-white px-4 focus:ring-2 focus:ring-primary-100 focus:border-primary-500 transition-all outline-none"
        >
            {options.map(opt => (
