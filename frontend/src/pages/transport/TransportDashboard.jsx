...
import {
    ...
    Bus, ArrowUpRight, ArrowDownRight, Activity,
    CreditCard, AlertCircle, CheckCircle, Clock, MapPin
} from 'lucide-react';
import {
...
const TransportDashboard = () => {
        ...
        <div className="h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800">
            <div className="flex flex-col items-center gap-4">
                <div className="relative">
                    ...
                    <Bus className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600" size={24} />
                </div>
    ...
    const collectionRate = (stats.billed && stats.billed > 0) ? ((stats.collected / stats.billed) * 100).toFixed(1) : 0;
    ...
    return (
        <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900 pb-20">
            {/* Header */}
                    ...
                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                        <div>
                            ...
                            <p className="text-slate-600 dark:text-slate-400 flex items-center gap-2">
                                ...
                                Overview of transport services and finances
                            </p>
                        </div>
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-6 space-y-8">
                    ...
                    <div className="lg:col-span-2 bg-white dark:bg-slate-800 rounded-2xl p-6 shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700">
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Active students by route</p>
                            </div>
                        </div>

                        <div className="h-80 w-full min-h-[320px]">
                            <ResponsiveContainer width="100%" height={320} minHeight={320}>
                                <BarChart data={(data.popular_routes && data.popular_routes.length > 0) ? data.popular_routes : [{ name: 'N/A', value: 0 }]} margin={{ top: 20, right: 30, left: 0, bottom: 60 }}>
                                    <XAxis
                                        dataKey="name"
                                        angle={-45}
                                        textAnchor="end"
                                    ...
                                    <Bar dataKey="value" radius={[8, 8, 0, 0]}>
                                        {((data.popular_routes && data.popular_routes.length > 0) ? data.popular_routes : [{ value: 0 }]).map((entry, index) => (
                ...
                <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-lg shadow-slate-200/50 dark:shadow-none border border-slate-100 dark:border-slate-700 overflow-hidden">
                    <div className="p-6 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-blue-50/50 to-transparent dark:from-blue-900/10">
                        <h3 className="text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                            <Clock size={20} className="text-blue-500" />
                            Recent Activity
                        </h3>
                        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">Latest transport transactions</p>
                    </div>
                    ...
                    <div className="overflow-x-auto">
                        ...
                        {(!data.recent_transactions || data.recent_transactions.length === 0) && (
                            <div className="py-8 px-6 text-left">
                                <p className="text-slate-400 text-xs italic">No transport ledger entries found for this period.</p>
                            </div>
                        )}
                    </div>
                </div>
